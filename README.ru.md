...
Гайд начинает настройку с получения собственных Telegram application credentials на [my.telegram.org/apps](https://my.telegram.org/apps): показывает вход в `API development tools`, объясняет поля формы и отдельно предупреждает, что API ID/API hash — не токен BotFather. Агент должен провести владельца по этим действиям, но не просить присылать значения.

Для новых задач и других agent harness навык также делает runtime access preflight: заранее сопоставляет приватный runtime с текущими workspace/permission roots и сразу выбирает узкое нативное разрешение точной команды. Это убирает лишний пробный `PermissionError`, но не отключает sandbox и не расширяет права Telegram-операций.
