...
`setup_account.py` запросит API ID, API hash и телефон только в локальном терминале; API hash не отображается, а телефон виден, чтобы проверить его перед сохранением. По умолчанию он не перезаписывает существующий профиль. Для осознанной замены используй `--replace` и введи `REPLACE` в том же терминале.
