...
class EventStore:
    """Small durable inbox for Telegram events and webhook delivery state."""
    ...
    def mark_webhook_failed(self, identifier: str, *, error: str, next_attempt_at: str) -> None:
        self.connection.execute(
            """
            UPDATE events
            SET webhook_attempts = webhook_attempts + 1,
                webhook_last_error = ?, webhook_next_attempt_at = ?
            WHERE id = ?
            """,
            (error[:240], next_attempt_at, identifier),
        )
        self.connection.commit()
