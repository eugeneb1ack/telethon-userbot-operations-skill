...
The collector invokes Telegram `messages.TranscribeAudioRequest` for newly collected voice, audio, and video-note records. Use only complete native transcriptions; report incomplete coverage and never substitute external STT. Telegram can reject a recording as `MSG_VOICE_TOO_LONG`; classify that message as `unsupported` with `error_code=voice_too_long`, do not retry it indefinitely, and do not claim that it was transcribed. The native API has no segment parameter, so do not silently upload split chunks. For photos, preview exact IDs with `download_media.py`, execute into a unique runtime subdirectory, inspect every verified image with `view_image`, and add only conversation-relevant visual context. Do not download, play, transcribe, or visually analyze video files.

The STT queue is bounded and resumable. `dialog_updates_native.py` uses two native workers by default (configurable from 1 to 4); `summarize_chat_native.py` keeps its explicit `--concurrency` setting. Retry only transient RPC/timeouts and flood waits; deterministic input errors and unsupported lengths are terminal for that message. A stable output plus `--resume` preserves completed and explicitly unsupported items; the progress JSONL contains status only, not transcript text. Do not run another direct helper for that account while collection is active.

## Live dialog freshness requests

For “последнее”, “ещё одно”, “следующее”, “сейчас”, “после моего сообщения” and equivalent freshness-sensitive requests, do not select targets from dialog-summary memory or an earlier metadata scan. Route to `dialog_updates_native.py` with the exact sender and one semantic selector: `--latest N`, `--unseen`, `--after-message-id`, or `--after-latest-outgoing`; choose `--content all|voice|text`. Keep the intents separate: “новые”, “непрослушанные”, “нерасшифрованные”, “с прошлого раза”, “ещё накидала” use `--unseen --content all` and then summarize only the requested media kinds; literal “после моего сообщения/голосового” uses `--after-latest-outgoing`. The latter is now incremental when a delivery cursor exists: its effective lower bound is the newer of the latest outgoing message and the saved cursor, so a repeated request cannot replay already delivered records. In private chats Telethon may ignore `from_user` at the API level and apply it client-side, so `--latest N` must scan a bounded window before selecting the sender's latest N records; never use N as the total network limit. The module enforces `message_id > anchor` before STT and `message_id > high_watermark` before merging a tail recheck, freezes IDs, performs native STT, rechecks for later text and media, and advances a content-scoped ID-only cursor only after the result is complete and current. If `complete=false`, automatically rerun or report that the tail is still moving; never present those records as the final latest slice. Exact known-ID media requests continue to use `transcribe_audio_native.py --message-id`.
